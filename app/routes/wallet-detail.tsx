...
async function fetchWalletAssets(wallet: Wallet): Promise<AssetOption[]> { const patterns = await paymentScriptPatterns(wallet); if (!patterns.length) throw new Error("Could not derive wallet script hash."); const res = await fetch(`/api/cardano/assets?patterns=${encodeURIComponent(patterns.join(","))}`); const body = await res.json(); if (!res.ok) throw new Error(body.error || "Could not load multisig assets."); return body.assets || []; }
...
export default function WalletDetail() {
  ...
  async function refreshAssets(target = wallet) { if (!target) return; setAssetStatus("Loading multisig assets from Kupo…"); try { const assets = await fetchWalletAssets(target); setWalletAssets(assets); setAssetStatus(assets.length ? `Loaded ${assets.length} multisig asset${assets.length === 1 ? "" : "s"}.` : "No spendable multisig assets found yet."); } catch (error) { setWalletAssets([]); setAssetStatus(error instanceof Error ? error.message : "Could not load multisig assets."); } }
  ...
  return <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
    ...
    <Card className="glass-panel"><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>Multisig assets</CardTitle><CardDescription>Fetched from the server-managed Kupo indexer for this script wallet.</CardDescription></div><Button variant="secondary" size="sm" onClick={() => refreshAssets()}><RefreshCw className="size-4" /> Refresh</Button></div></CardHeader><CardContent>{walletAssets.length === 0 ? <div className="rounded-lg border border-border bg-slate-950/60 p-4 text-sm text-slate-400"><Database className="mr-2 inline size-4 text-sky-300" /> {assetStatus}</div> : <div className="grid gap-3 md:grid-cols-3">{walletAssets.map((asset) => <div className="rounded-lg border border-border bg-slate-950/60 p-4" key={asset.unit}><div className="text-sm text-slate-400">{asset.label}</div><div className="mt-1 font-mono text-lg font-semibold text-slate-100">{formatRawQuantity(asset.quantity, asset.unit, asset.decimals)}</div><div className="mt-1 truncate text-xs text-slate-500">{asset.unit}</div></div>)}</div>}<div className="mt-3 text-xs text-slate-500">{assetStatus}</div></CardContent></Card>
