...
export default function WalletDetail() {
  ...
  async function connectSigner(provider: WalletProvider) { if (connecting) return; setConnecting(provider.id); setSignStatus(`Open ${provider.name} and approve the connection...`); try { const api = await withTimeout(provider.enable(), 12000, `${provider.name} did not answer. Unlock the wallet popup, then try again.`); let networkId = -1; let keyHash: string | null = null; try { networkId = await withTimeout(api.getNetworkId(), 5000, "Network lookup timed out."); } catch {} try { const used = await withTimeout(api.getUsedAddresses(), 5000, "Address lookup timed out."); const address = used[0] || await withTimeout(api.getChangeAddress(), 5000, "Change address lookup timed out."); keyHash = address ? await keyHashFromAddress(address) : null; } catch {} setConnected({ id: provider.id, name: provider.name, api, networkId, keyHash }); setSignStatus(`Connected ${provider.name}${keyHash ? ` · ${keyHash.slice(0, 10)}…` : ""}.`); } catch (error) { setSignStatus(error instanceof Error ? error.message : `Could not connect ${provider.name}.`); } finally { setConnecting(null); } }
  async function signTransaction(tx: TxDraft) { if (!connected) { setSignStatus("Connect a signer wallet first."); return; } if (!tx.unsignedTxCbor?.trim()) { setSignStatus("This transaction has no unsigned tx CBOR, so a wallet cannot sign it yet. Create/paste the unsigned transaction CBOR first."); return; } try { setSignStatus(`Requesting ${connected.name} signature…`); const witnessCbor = await connected.api.signTx(tx.unsignedTxCbor.trim(), true); const signerKeyHash = connected.keyHash?.toLowerCase() || `unknown-${connected.id}`; const signature: SignatureRecord = { signerKeyHash, signerName: connected.keyHash || connected.name, walletName: connected.name, witnessCbor, signedAt: new Date().toISOString() }; const next = txs.map((item) => item.id === tx.id ? { ...item, signatures: [...(item.signatures || []).filter((sig) => sig.signerKeyHash !== signerKeyHash), signature], status: "pending" as TxStatus, updatedAt: new Date().toISOString() } : item); setTxs(next); writeArray(TX_KEY, next); setSignStatus("Signature captured. Export/copy the witness package for the coordinator or continue collecting signatures here."); } catch (error) { setSignStatus(error instanceof Error ? error.message : "Wallet refused to sign."); } }
