...
async function fetchMultisigAssets(wallet: Wallet): Promise<AssetOption[]> { const patterns = await paymentScriptPatterns(wallet); if (!patterns.length) throw new Error("Could not derive the wallet script hash yet. Re-import the wallet if this persists."); const res = await fetch(`/api/cardano/assets?patterns=${encodeURIComponent(patterns.join(","))}`); const body = await res.json(); if (!res.ok) throw new Error(body.error || "Could not fetch multisig assets."); return (body.assets || []).map((asset: { unit: string; label: string; quantity: string; outputCount?: number; decimals?: number }) => ({ ...asset, source: "treasury" as const })); }
...
export default function NewTransaction() {
  ...
  async function refreshMultisigAssets(target = wallet) { if (!target) return; setAssetStatus("Loading multisig assets from Kupo…"); try { const fetched = await fetchMultisigAssets(target); setMultisigAssets(fetched.length ? fetched : [DEFAULT_ASSET]); setAssetStatus(fetched.length ? `Loaded ${fetched.length} multisig asset${fetched.length === 1 ? "" : "s"} from Kupo.` : "No spendable multisig assets found yet."); } catch (error) { setMultisigAssets([DEFAULT_ASSET]); setAssetStatus(error instanceof Error ? error.message : "Could not fetch multisig assets."); } }
