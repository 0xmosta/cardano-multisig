...
export default function WalletDetail() {
  ...
  useEffect(() => { if (wallet) setHandleInput(wallet.handle || ""); }, [wallet?.id]);
  ...
  function saveHandle() { if (!wallet) return; const clean = handleInput.trim().replace(/^\$/, ""); const next = wallets.map((item) => item.id === wallet.id ? { ...item, handle: clean || undefined } : item); setWallets(next); writeArray(WALLET_KEY, next); void refreshAssets({ ...wallet, handle: clean || undefined }); }
  ...
  return <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
    ...
    <Card className="glass-panel"><CardHeader><CardTitle>Wallet identity</CardTitle><CardDescription>Resolve the treasury ADA Handle so balances come from the exact multisig address, not a fallback script pattern.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-[1fr_auto]"><div className="space-y-2"><Label>ADA Handle</Label><Input value={handleInput} onChange={(event) => setHandleInput(event.target.value)} placeholder="$discatalyst" /></div><div className="flex items-end"><Button variant="secondary" onClick={saveHandle}>Resolve & save</Button></div>{resolvedHandle ? <div className="md:col-span-2 truncate rounded-lg border border-emerald-400/20 bg-emerald-400/10 p-3 text-sm text-emerald-100">Resolved {handleLabel(resolvedHandle)} → {resolvedHandle.address}</div> : null}</CardContent></Card>
