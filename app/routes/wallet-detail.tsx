...
async function fetchWalletAssets(wallet: Wallet): Promise<AssetFetch> { const patterns = await paymentScriptPatterns(wallet); const query = assetQuery(patterns, wallet); if (!query) throw new Error("Could not derive wallet script hash or ADA Handle."); const res = await fetch(`/api/cardano/assets?${query}`); const body = await res.json(); if (!res.ok) throw new Error(body.error || "Could not load multisig assets."); return { assets: body.assets || [], handle: body.handle, source: body.source, address: body.address, outputs: body.outputs }; }
