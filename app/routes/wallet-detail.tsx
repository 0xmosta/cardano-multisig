...
export default function WalletDetail() {
  ...
  return <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
    ...
    <Card className="glass-panel"><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>Multisig assets</CardTitle><CardDescription>{resolvedHandle ? `ADA Handle ${handleLabel(resolvedHandle)} resolved to this multisig script address.` : "Fetched from the server-managed Cardano provider for this script wallet."}</CardDescription></div><Button variant="secondary" size="sm" onClick={() => refreshAssets()}><RefreshCw className="size-4" /> Refresh</Button></div></CardHeader><CardContent>{walletAssets.length === 0 ? <div className="rounded-lg border border-border bg-slate-950/60 p-4 text-sm text-slate-400"><Database className="mr-2 inline size-4 text-sky-300" /> {assetStatus}</div> : <div className="grid gap-3 md:grid-cols-3">{walletAssets.map((asset) => <div className="rounded-lg border border-border bg-slate-950/60 p-4" key={asset.unit}><div className="text-sm text-slate-400">{asset.label}</div><div className="mt-1 font-mono text-lg font-semibold text-slate-100">{formatRawQuantity(asset.quantity, asset.unit, asset.decimals)}</div><div className="mt-1 truncate text-xs text-slate-500">{asset.unit}</div></div>)}</div>}<div className="mt-3 text-xs text-slate-500">{assetStatus}</div></CardContent></Card>
