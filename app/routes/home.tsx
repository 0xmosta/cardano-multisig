...
type ServerProviderStatus = { mode: "server"; network: Network; ready: boolean; services: { blockfrost: boolean; kupo: boolean; ogmios: boolean; submit: boolean } };
...
const DEFAULT_NETWORK: Network = import.meta.env.VITE_CARDANO_NETWORK === "mainnet" || import.meta.env.VITE_CARDANO_NETWORK === "preview"
...
function migrateWallet(raw: unknown): MultisigWallet | null {
  ...
  return { id: typeof raw.id === "string" ? raw.id : createId("wallet"), name: raw.name, handle: typeof raw.handle === "string" ? raw.handle : undefined, network: NETWORKS.includes(raw.network as Network) ? raw.network as Network : DEFAULT_NETWORK, threshold: typeof raw.threshold === "number" ? raw.threshold : requiredSignatures(script), signers, paymentScript: isRecord(raw.paymentScript) ? raw.paymentScript as NativeScript : script, stakeScript: isRecord(raw.stakeScript) ? raw.stakeScript as NativeScript : null, script, createdAt: typeof raw.createdAt === "string" ? raw.createdAt : nowIso(), imported: Boolean(raw.imported) };
}
...
export default function Home() {
  ...
  function importWallet() { if (!canImport || !parsedPayment.script) return; const handle = importHandle.trim().replace(/^\$/, ""); const wallet: MultisigWallet = { id: createId("wallet"), name: handle ? `$${handle}` : "Imported wallet", handle: handle || undefined, network: DEFAULT_NETWORK, threshold: importThreshold, signers: importedSigners, paymentScript: parsedPayment.script, stakeScript: parsedStake.script, script: parsedPayment.script, createdAt: nowIso(), imported: true }; setWallets((current) => [wallet, ...current]); }
  function saveCreatedWallet() { if (!canSave) return; const wallet: MultisigWallet = { id: createId("wallet"), name: "New multisig wallet", network: DEFAULT_NETWORK, threshold: clampedThreshold, signers: validSigners, paymentScript: draftScript, stakeScript: null, script: draftScript, createdAt: nowIso(), imported: false }; setWallets((current) => [wallet, ...current]); }
